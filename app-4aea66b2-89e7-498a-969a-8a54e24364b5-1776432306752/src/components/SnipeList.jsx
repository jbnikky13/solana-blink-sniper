...
const STATUS_STYLES = {
  watching: {
    label: 'Watching',
    cls: 'bg-brand-accent/15 text-brand-accent border-brand-accent/30',
    Icon: Eye,
  },
  sniped: {
    label: 'Sniped',
    cls: 'bg-brand-green/15 text-brand-green border-brand-green/30',
    Icon: CheckCircle,
  },
  dismissed: {
    label: 'Dismissed',
    cls: 'bg-brand-red/15 text-brand-red border-brand-red/30',
    Icon: XCircle,
  },
};
...
export default function SnipeList({ targets, onUpdateStatus, onDelete }) {
  ...
  const filtered =
    filter === 'all' ? targets : targets.filter(function (t) { return t.status === filter; });
      ...
      {filtered.length === 0 ? (
        <div className="p-8 text-center">
          ...
          <p className="text-brand-muted text-xs mt-1">
            Click the snipe icon on a transaction with token activity.
          </p>
        </div>
      ) : (
...
function SnipeRow({ target, onUpdateStatus, onDelete }) {
  ...
  var age = target.created_at
    ? formatDistanceToNow(new Date(target.created_at), { addSuffix: true })
    : '';

  return (
    <div className="px-4 py-3 flex flex-col gap-2 hover:bg-white/5 transition-colors text-xs group">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          {target.token_name && (
            <p className="text-brand-subtext mt-0.5">
              {target.token_name}{target.token_symbol ? ' (' + target.token_symbol + ')' : ''}
            </p>
          )}

          <p className="text-brand-muted mt-0.5">
            {'Slot ' + (target.slot ? target.slot.toLocaleString() : '-') + ' · ' + age}
          </p>
        </div>

        <span
          className={'px-2 py-0.5 rounded border text-[10px] font-semibold tracking-wide flex items-center gap-1 ' + style.cls}
        >
          <StatusIcon size={9} />
          {style.label}
        </span>
        ...
        <div className="flex items-center gap-1">
          {target.status !== 'watching' && (
            <button
              onClick={function () { onUpdateStatus(target.id, 'watching'); }}
          ...
          {target.status !== 'sniped' && (
            <button
              onClick={function () { onUpdateStatus(target.id, 'sniped'); }}
          ...
          {target.status !== 'dismissed' && (
            <button
              onClick={function () { onUpdateStatus(target.id, 'dismissed'); }}
          ...
          <button
            onClick={function () { onDelete(target.id); }}
      ...
      <div className="flex gap-2 pt-1 border-t border-brand-border/50">
        <a
          href={'https://twitter.com/search?q=' + target.token_address}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-blue-500 hover:bg-blue-400 text-[10px] px-2 py-1 rounded text-white transition-colors"
        >
          Twitter
        </a>
        <a
          href={'https://t.me/s/search?q=' + target.token_address}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-blue-700 hover:bg-blue-600 text-[10px] px-2 py-1 rounded text-white transition-colors"
        >
          Telegram
        </a>
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="bg-pink-600 hover:bg-pink-500 text-[10px] px-2 py-1 rounded text-white transition-colors"
        >
          DexScreener
        </a>
      </div>
