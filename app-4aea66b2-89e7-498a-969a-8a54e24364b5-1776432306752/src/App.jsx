...
export default function App() {
  ...
  const [snipeModal, setSnipeModal] = useState(null);
  const [toast, setToast] = useState(null);
  const [activeTab, setActiveTab] = useState('transactions');
  const [marketDetails, setMarketDetails] = useState({});
  const [earlyTokens, setEarlyTokens] = useState([]);
  ...
  useEffect(() => {
    const updateMarketData = async () => {
      ...
      if (latestTx?.mint && !marketDetails[latestTx.mint]) {
        ...
        if (data) setMarketDetails(prev => ({ ...prev, [latestTx.mint]: data }));
      }
    };
    updateMarketData();
  }, [transactions]);
  ...
  const loadTargets = useCallback(async () => {
    ...
    const { data, error: err } = await supabase.from('snipe_targets').select('*').order('created_at', { ascending: false });
  ...
  const runEarlyScan = useCallback(async () => {
    ...
    setEarlyError(null);
    try {
      const result = await runEarlyGemScan();
      setEarlyTokens(result.tokens || []);
      setTrendingWords(result.trendingWords || []);
    } catch (err) {
      setEarlyError(err.message || 'Early-token scan failed');
    } finally {
  ...
  const handleSnipeConfirm = async ({ tx, mint, notes }) => {
    ...
    if (err) showToast('Failed to save target: ' + err.message, 'error');
    else {
      setTargets(prev => [data, ...prev]);
      setSnipeModal(null);
      showToast('Snipe target added!');
      setActiveTab('targets');
    }
  };

  const handleUpdateStatus = async (id, status) => {
    const { error: err } = await supabase.from('snipe_targets').update({ status, updated_at: new Date().toISOString() }).eq('id', id);
    if (err) showToast('Update failed: ' + err.message, 'error');
    else setTargets(prev => prev.map(t => t.id === id ? { ...t, status } : t));
  };

  const handleDelete = async (id) => {
    const { error: err } = await supabase.from('snipe_targets').delete().eq('id', id);
    if (err) showToast('Delete failed: ' + err.message, 'error');
    else { setTargets(prev => prev.filter(t => t.id !== id)); showToast('Target removed.'); }
  };
  ...
  const tabs = [
    ...
    { id: 'early', label: 'Early Radar', count: earlyTokens.length },
  ];
    ...
    <main className="flex-1 max-w-screen-2xl mx-auto w-full px-4 py-5 space-y-5">
      ...
      <div className="flex items-center gap-1 border-b border-brand-border overflow-x-auto">
        {tabs.map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`whitespace-nowrap px-4 py-2.5 text-sm font-medium transition-colors flex items-center gap-2 border-b-2 -mb-px ${activeTab === tab.id ? 'border-brand-accent text-white' : 'border-transparent text-brand-subtext hover:text-white hover:border-brand-border'}`}>
          {tab.label}{tab.count > 0 && <span className={`text-xs rounded-full px-1.5 py-0.5 font-mono ${activeTab === tab.id ? 'bg-brand-accent text-black' : 'bg-brand-border text-brand-subtext'}`}>{tab.count}</span>}
        </button>)}
      </div>
